...
# === 路径设置 ===
input_file = 'data_ETH_daily.csv'
output_folder_kline = './ETH_KLine_Segments'
output_folder_ma = './ETH_MA_Segments'
...
for idx, (start_i, end_i, segment_df) in enumerate(segments):
    ...
    for i in range(len(segment_df)):
        ...
        # 高低线
        ax.plot([x_labels[i], x_labels[i]], [l, h], color=color, linewidth=2)
        # 实体线加粗
        ax.plot([x_labels[i], x_labels[i]], [o, c], color=color, linewidth=8)
    ...
    plt.tight_layout()
    output_path_kline = os.path.join(output_folder_kline, f'BTC_KLine_{idx+1}_{segment_df["Date"].iloc[0].date()}_{segment_df["Date"].iloc[-1].date()}.png')
    ...
    plt.tight_layout()
    output_path_ma = os.path.join(output_folder_ma, f'BTC_MA_{idx+1}_{segment_df["Date"].iloc[0].date()}_{segment_df["Date"].iloc[-1].date()}.png')
